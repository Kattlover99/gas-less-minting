...
describe("GASL",   function() {
  async function deploy() {
    const [minter, redeemer, _] = await ethers.getSigners()
  
    let factory = await ethers.getContractFactory("GASL", minter)
  ...
  //   const GASL = await GASL.deploy(minter);
  //   await GASL.deployed();
  ...
  it("Should redeem an NFT from a signed voucher", async function() {
    ...
    await expect(redeemerContract.redeem(redeemer, voucher, signature))
      .to.emit(contract, 'Transfer')  // transfer from null address to minter
      .withArgs('0x0000000000000000000000000000000000000000', minter.address, voucher.tokenId)
      .and.to.emit(contract, 'Transfer') // transfer from minter to redeemer
      .withArgs(minter.address, redeemer, voucher.tokenId);
  });
